...
const cartReducer = (state, action) => {
  ...
  switch (action.type) {
    case "ADD":
      ...
      products.indexOf(action.item) === -1 ? 
      (newCart.products.push(action.item), action.item.amount = 1) : action.item.amount += 1;
      // 增加購物車中的商品總數量
    ...
    case "REMOVE":
      ...
      if (action.item.amount === 0) {
        newCart.products.splice(newCart.products.indexOf(action.item), 1)
      }
      // 修改商品總數及總金額
      newCart.totalAmount -= 1;
      // 增加購物車中的總金額
      ...
      return newCart;
    case "REMOVEALL":
      newCart.products.forEach(item => delete item.amount)
      newCart.products = [];
